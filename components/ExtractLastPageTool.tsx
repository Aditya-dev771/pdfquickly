...
export default function ExtractLastPageTool() {
  ...
  async function extractLastPage() {
    if (!file) {
    ...
    try {
      ...
      const [lastPage] = await newPdf.copyPages(pdf, [pageCount - 1]);
  ...
  type: "application/pdf",
      ...
      link.href = url;
      link.download = "last-page.pdf";
      link.click();

      URL.revokeObjectURL(url);
    } catch (error) {
      console.error(error);
      alert("Failed to extract last page.");
    } finally {
      setLoading(false);
    }
  }
